import { teamMatchResultSchema } from "./theme.js";
...
export function generateTeamAliases(team) {
...
function buildCandidates(inputName, teams) {
    ...
    return teams
        ...
        if (normalizedInput && normalizedInput === normalizedAlias) {
            ...
            confidence = Math.max(diceCoefficient(normalizedInput, normalizedAlias) * 0.92, diceCoefficient(compactInput, compactAlias) * 0.95);
        }
        return {
            team,
            alias,
...
export function matchTeamName(inputName, teams) {
    const normalizedInput = normalizeTeamName(inputName);
    if (!normalizedInput) {
