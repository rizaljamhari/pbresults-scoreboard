...
import { appExportSchema, assetSchema, createThemeId, settingsSchema, teamMatchResultSchema, teamRecordSchema, themeExportSchema, themeSchema } from "../shared/theme.js";
...
function collectThemeAssetIds(theme) {
...
function remapThemeAssetIds(theme, idMap) {
    for (const component of Object.values(theme.components)) {
...
function withBuiltinThemes(themes) {
    const byId = new Map(themes
...
function tryMigrateLegacyDatabase() {
    if (!fs.existsSync(legacyDatabasePath)) {
        return false;
    }
    try {
        const settingsRows = JSON.parse(execFileSync("sqlite3", ["-json", legacyDatabasePath, "SELECT key, value FROM settings"]).toString());
        const themeRows = JSON.parse(execFileSync("sqlite3", ["-json", legacyDatabasePath, "SELECT data FROM themes ORDER BY builtin DESC, name ASC"]).toString());
        const assetRows = JSON.parse(execFileSync("sqlite3", ["-json", legacyDatabasePath, "SELECT id, original_name, mime_type, url, file_path, created_at FROM assets ORDER BY created_at DESC"]).toString());
        const settingsRaw = {};
...
export function getSettings() {
    ...
    const themeIds = new Set(withBuiltinThemes(readJson(themesPath, [])).map((theme) => theme.id));
...
export function listThemes() {
    return withBuiltinThemes(readJson(themesPath, []))
        .map((theme) => themeSchema.parse(theme))
        .sort((left, right) => {
...
export function getTeamRecord(id) {
...
export function saveTeamRecord(team) {
...
export function deleteTeamRecord(id) {
    writeJson(teamsPath, listTeamRecords().filter((team) => team.id !== id));
}
export async function attachTeamLogo(teamId, buffer, originalName, mimeType, slot = "primary") {
...
export function matchTeamInput(inputName) {
...
export function createThemeFromClone(cloneFromId, name) {
...
export function deleteTheme(id) {
    ...
    writeJson(themesPath, themes.filter((theme) => theme.id !== id));
...
export function getAsset(id) {
    return readJson(assetsPath, []).find((asset) => asset.id === id) ?? null;
}
export async function storeAsset(buffer, originalName, mimeType) {
...
export async function exportThemePackage(id) {
    const theme = getTheme(id);
    if (!theme) {
        throw new Error("Theme not found");
    }
...
export async function importThemePackage(pkg) {
...
export async function exportAppPackage() {
    ...
    const themes = listThemes();
    const teams = listTeamRecords();
    const assets = await Promise.all(readJson(assetsPath, []).map(async (asset) => {
        const file = await fsp.readFile(asset.filePath);
        return {
...
export async function importAppPackage(pkg) {
