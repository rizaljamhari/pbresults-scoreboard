...
import { attachTeamLogo, createTeamRecord, deleteTheme, deleteTeamRecord, exportAppPackage, exportThemePackage, getSettings, getTheme, getTeamRecord, importAppPackage, importThemePackage, listAssets, listTeamRecords, listThemes, matchTeamInput, publishTheme, saveTeamRecord, saveTheme, storeAsset, updateSettings, createThemeFromClone } from "./storage.js";
...
app.get("/api/teams/:id", async (request, reply) => {
    const team = getTeamRecord(request.params.id);
    if (!team) {
...
app.delete("/api/teams/:id", async (request, reply) => {
    deleteTeamRecord(request.params.id);
    livePoller.reconfigure();
...
app.post("/api/teams/:id/logo", async (request, reply) => {
    ...
    const slot = (request.query?.slot ?? "primary") === "alternate" ? "alternate" : "primary";
    try {
        const result = await attachTeamLogo(request.params.id, await file.toBuffer(), file.filename, file.mimetype, slot);
        livePoller.reconfigure();
...
app.get("/api/themes/:id", async (request, reply) => {
    const theme = getTheme(request.params.id);
    if (!theme) {
...
app.get("/api/themes/:id/export", async (request, reply) => {
    try {
        const pkg = await exportThemePackage(request.params.id);
